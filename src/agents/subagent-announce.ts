...
async function buildSubagentStatsLine(params: {
  ...
  const result = `Stats: ${parts.join(" \u2022 ")}`;

  // Truncate if exceeds terminal width (pi-tui crashes if lines are too long)
  const MAX_STATS_WIDTH = 150;
  if (visibleWidth(result) > MAX_STATS_WIDTH) {
    // Find parts that can be safely truncated
    const truncatedParts = parts.map((part) => {
      // Truncate sessionId (UUID) if present
      if (part.startsWith("sessionId ")) {
        const id = part.slice(10);
        const truncatedId = id.length > 16 ? `${id.slice(0, 8)}…${id.slice(-6)}` : id;
        return `sessionId ${truncatedId}`;
      }
      // Truncate transcript path if present
      if (part.startsWith("transcript ")) {
        const path = part.slice(11);
        const truncatedPath = path.length > 30 ? `${path.slice(0, 20)}…${path.slice(-8)}` : path;
        return `transcript ${truncatedPath}`;
      }
      // Truncate sessionKey if needed
      if (part.startsWith("sessionKey ")) {
        const key = part.slice(11);
        if (key.length > 40) {
          return `sessionKey ${key.slice(0, 25)}…${key.slice(-12)}`;
        }
      }
      return part;
    });

    const truncatedResult = `Stats: ${truncatedParts.join(" \u2022 ")}`;
    // If still too long, progressively truncate more
    if (visibleWidth(truncatedResult) > MAX_STATS_WIDTH) {
      const minimalParts = parts.map((part) => {
        if (part.startsWith("sessionId ")) {
          const id = part.slice(10);
          return id.length > 8 ? `sessionId ${id.slice(0, 6)}…${id.slice(-4)}` : part;
        }
        if (part.startsWith("transcript ")) {
          return "transcript …";
        }
        if (part.startsWith("sessionKey ")) {
          const key = part.slice(11);
          return key.length > 30 ? `sessionKey ${key.slice(0, 15)}…${key.slice(-8)}` : part;
        }
        return part;
      });
      return `Stats: ${minimalParts.join(" \u2022 ")}`;
    }
